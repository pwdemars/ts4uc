...
for epoch in "final" 290000 ;
	do for rho in 01 02 05 10 ; 
		do for num_scenarios in 100 200 ;
	  	do let secs=$t*48*3+500 && \
	     time=$(date -d@$secs -u +%H:%M:%S) && \
	     qsub -l h_rt=$time submit_anytime_tree_search.sh $HOME/Scratch/results/${date}_307/guided_${tree_search_func_name}/g${g}_p${rho}_s${num_scenarios}_e${epoch} \
