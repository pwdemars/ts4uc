		...
		do for H in 2 4 ; 
			do for rho in 05;
				do for heuristic_method in "check_lost_load" "simple_priority_list" "advanced_priority_list" "none";
					do qsub -l h_rt=12:00:00 submit_tree_search.sh $HOME/Scratch/results/${date}_201/guided_${tree_search_func_name}/g${g}/feb4_g${g}_h${H}_p${rho}_${heuristic_method} \
		...
		do for H in 6 8; 
			do for rho in 05;
				do for heuristic_method in "advanced_priority_list";
					do qsub -l h_rt=16:00:00 submit_tree_search.sh $HOME/Scratch/results/${date}_201/guided_${tree_search_func_name}/g${g}/feb4_g${g}_h${H}_p${rho}_${heuristic_method} \
																 $HOME/AISO_HPC/best_policies/g${g}/params.json \
																 $HOME/ts4uc/data/day_ahead/${g}gen/30min/env_params.json \
																 $HOME/AISO_HPC/best_policies/g${g}/ac_final.pt \
																 ${H} \
