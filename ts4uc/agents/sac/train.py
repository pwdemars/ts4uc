...
class ReplayBuffer:
    ...
    def store(self, obs=None, act=None, rew=None, next_obs=None, done=None):
    ...
    def get(self):
        ...
        data = dict(obs=self.obs_buf, act=self.act_buf,
                    rew=self.rew_buf, next_obs=self.next_obs_buf,
                    done=self.done_buf)
	...
	def run_epoch(self):
		...
		while sum(all_ep_timesteps) <= self.steps_per_epoch: 
			...
			while not done:
				...
				# 	acts.append(a[i])
				# 	obss.append(sub_obs[i])
				# 	rews.append(reward)
				# 	if done and ((i+1) == len(a)):
				# 		last_obs = self.policy.obs_with_constraints(env, obs)
				# 		next_obss.append(last_obs)
				# 	else:
				# 		next_obss.append(sub_obs[i+1])
				# 	dones.append(done)
...
def train(save_dir,
	...
	buf = ReplayBuffer(max_size=num_workers*steps_per_epoch*env.num_gen, obs_dim=policy.n_in)
